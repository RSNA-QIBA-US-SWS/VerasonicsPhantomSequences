...
function errorFlag = MakePlotAndSaveOutputCSVfile(par)
    ...
    for ifile = 1:length(analysisFiles)         % load Vdisp, Vvel, phVel data
        ...
        S = load([analysisFiles(ifile).folder '/' analysisFiles(ifile).name], 'Vdisp', 'Vvel', 'freqsToAnalyzeHz', 'phVel');
        Vdisp(ifile)     = S.Vdisp;
        Vvel(ifile)      = S.Vvel;
        phVel(ifile, :)  = S.phVel;
    ...
    savefig([analysisDir '/gSWS_phVel_figure'])
    saveas(gcf,[analysisDir '/gSWS_phVel_figure.png'])
    
    % make and save CSV file
    saveFile = [analysisDir '/gSWS_phVel_data.txt'];
    ...
    fprintf(fid, 'displacement,%s,%s,%s\n', sprintf('%7.3f', Vdisp_avg),...
                                            sprintf('%7.3f', CIfactor*Vdisp_std),...
                                            sprintf('%7.3f', fracErrorBar*Vdisp_avg) );
    fprintf(fid, 'velocity,%s,%s,%s\n',     sprintf('%7.3f', Vvel_avg),...
                                            sprintf('%7.3f', CIfactor*Vvel_std),...
                                            sprintf('%7.3f', fracErrorBar*Vvel_avg) );
    fprintf(fid, '\n');
    fprintf(fid, '\n');
    ...
    for ii = 1:length(fidx)
        fprintf(fid, '%s,%s,%s,%s\n', sprintf('%7.0f', freqsToAnalyzeHz(fidx(ii))),...
                                      ...
                                      sprintf('%7.3f', CIfactor*phVel_std(fidx(ii))),...
                                      sprintf('%7.3f', fracErrorBar*phVel_avg(fidx(ii))) );
    end
    fclose(fid);
