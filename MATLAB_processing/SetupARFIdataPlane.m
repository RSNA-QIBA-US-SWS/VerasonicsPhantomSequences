...
function [dispPlane,tms,latmm] = SetupARFIdataPlane(arfidata,axialmm,latmm,tms,pushFocusMM,fliplat,par)
    ...
    desiredPRFkHz   = par.desiredPRFkHz;
...
function [plane,tms,latmm] = RemoveAndFixReverb(plane,tms,latmm,nStepsToRemove)
    ...
    if length(tidx)~=1
        error('Only one timepoint in time trace')
    end
...
function output = HighPassFilterInTime2D(plane,t,fcutoff)
    ...
    filtorder=3;
    [b,a]=butter(filtorder,fnorm,'high');
