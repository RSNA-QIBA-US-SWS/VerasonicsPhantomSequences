...
def analyze_STPs_for_group_and_phase_velocity(output_disp_and_vel_STP_dict:dict, analysis_params:AnalysisPar.AnalysisPar, acq_params:DataAcqPar.DataAcqPar=None, saving:int=1):
    ...
    for plane_side in output_disp_and_vel_STP_dict.keys():
        ...
        if analysis_params.SWS_est_method == "radonsum":
            print('using radon sum')
            Vdisp = STPA.run_radon_sum_to_determine_group_vel(sub_dict['disp_plane'], sub_dict['disp_lat_mm'], sub_dict['disp_t_ms'], analysis_params.min_lat_mm, analysis_params.max_lat_mm,
                                analysis_params.min_time_ms, analysis_params.max_time_ms,  N=len(sub_dict['disp_t_ms']))
            
            Vvel = STPA.run_radon_sum_to_determine_group_vel(sub_dict['vel_plane'], sub_dict['vel_lat_mm'], sub_dict['vel_t_ms'], analysis_params.min_lat_mm, analysis_params.max_lat_mm,
                                analysis_params.min_time_ms, analysis_params.max_time_ms, N=len(sub_dict['vel_t_ms']))
        else:
            print('using TTP')
            # Run the TTP algorithm on the displacement-data spacetime plot to determine displacement group speed
        ...
        output_sub_dict = {
            ...
            "freqsToAnalyzeHz" : freqs_to_analyze,
...
def filter_SWSs_and_display_final_outputs(QIBA_output:QIBAOutput.QIBAOutput, plot_params:QIBAPlotPar.QIBAPlotPar):
    ...
    QIBA_output.generate_QIBA_output_plot(plot_params,save=1)
